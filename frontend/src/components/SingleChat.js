...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  //     const { data: messageData } = await axios.post("/api/message", imageMessage, config);
...
//     // 👉 Debug: Log image URL returned from backend
...
//     setMessages([...messages, messageData]);
//   } catch (error) {
//     console.log("❌ Upload error:", error); // 👈 useful for catching error cause
//     toast({
//       title: "Image upload failed",
//       status: "error",
//       duration: 5000,
...
const handleImageUpload = async (e) => {
  ...
  try {
    ...
    const messagePayload = { content, chatId };

    console.log("📨 Sending message with:", messagePayload);
    ...
    const { data: messageData } = await axios.post("/api/message", messagePayload, configMessage);
